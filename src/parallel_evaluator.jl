...
"""
struct PEOutputMessage{F}
    id::Int                   # candidate id
    fitness::F
end

const PEInputChannel = Channel{PEInputMessage}
const PEInputChannelRef = ChannelRef{PEInputMessage}

const PEOutputChannel{F} = Channel{PEOutputMessage{F}}
const PEOutputChannelRef{F} = ChannelRef{PEOutputMessage{F}}

"""
Internal data for the worker process of the parallel evaluator.
"""
struct ParallelEvaluatorWorker{F, P<:OptimizationProblem}
    id::Int
    problem::P
    in_individuals::PEInputChannelRef
    out_fitnesses::PEOutputChannelRef{F}

    ParallelEvaluatorWorker(id::Int, problem::P,
            in_individuals::PEInputChannelRef, out_fitnesses::PEOutputChannelRef{F}
    ) where {F, P<:OptimizationProblem} =
        new{F, P}(id, problem, in_individuals, out_fitnesses)
end

# run the wrapper (called in the "main" task)
function run!(worker::ParallelEvaluatorWorker) where F
...
function run_parallel_evaluator_worker(id::Int,
                    ...
                    in_individuals::PEInputChannelRef,
                    out_fitnesses::PEOutputChannelRef{F}) where F
...
mutable struct ParallelEvaluator{F, FA, FS, P<:OptimizationProblem, A<:Archive} <: Evaluator{P}
    ...
    ParallelEvaluator(
        ...
        out_individuals = RemoteChannel(() -> PEInputChannel(queueCapacity), myid())
        in_fitnesses = RemoteChannel(() -> PEOutputChannel{F}(5*queueCapacity), myid())
...
function check_worker_running(worker::ChannelRef)
    if isready(worker)
...
"""
function recv_fitnesses!(etor::ParallelEvaluator{F}) where F
    #info("recv_fitnesses()")
...
"""
function update_fitness!(etor::ParallelEvaluator{F,FA}, candidates::Vector{Candidate{FA}};
    ...
    isempty(fit_ids) || throw(InternalError("Fitnesses not evaluated ($fit_ids)"))
    return candidates
end
