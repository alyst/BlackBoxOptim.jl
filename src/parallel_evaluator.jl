...
typealias PEInputChannelRef ChannelRef{PEInputMessage}

typealias PEOutputChannel{F} Channel{PEOutputMessage{F}}
typealias PEOutputChannelRef{F} ChannelRef{PEOutputMessage{F}}
...
immutable ParallelEvaluatorWorker{F, P<:OptimizationProblem}
    ...
    @compat (::Type{ParallelEvaluatorWorker}){F, P<:OptimizationProblem}(id::Int, problem::P,
...
type ParallelEvaluator{F, FA, FS, P<:OptimizationProblem, A<:Archive} <: Evaluator{P}
    ...
    @compat (::Type{ParallelEvaluator}){P<:OptimizationProblem, A<:Archive}(
        ...
        out_individuals = @compat RemoteChannel(() -> PEInputChannel(queueCapacity), myid())
        in_fitnesses = @compat RemoteChannel(() -> PEOutputChannel{F}(5*queueCapacity), myid())
