...
const ParallelEvaluatorWorkerRef{T,P} = ChannelRef{ParallelEvaluatorWorker{T,P}}

@inline function (worker::ParallelEvaluatorWorker)()
    put_fitness!(worker.shared_fitness, fitness(worker.shared_param, worker.problem))
    return nothing
end

@inline function (worker_ref::ParallelEvaluatorWorkerRef{T,P})() where {T,P} =
...
mutable struct ParallelEvaluator{F, FA, T, FS, P<:OptimizationProblem, A<:Archive} <: Evaluator{P}
    ...
    ParallelEvaluator(
        ...
        worker_refs = [RemoteChannel(function ()
...
function update_fitness!(etor::ParallelEvaluator{F,FA}, candidates::Vector{Candidate{FA}};
    ...
    (n_pending > 0) && throw(InternalError("Fitnesses not evaluated ($job_ids)"))
    return candidates
end
