...
const PESlotChannel = Channel{Int} # channel for exchaning slot Ids
const PESlotChannelRef = ChannelRef{Int}
...
mutable struct ParallelEvaluator{F, FA, FS, P<:OptimizationProblem, A<:Archive} <: Evaluator{P}
    ...
    ParallelEvaluator(
        ...
        etor = new{F, FA, typeof(fs), P, A}(
            ...
            PESlotChannelRef[RemoteChannel(() -> PESlotChannel(paramChanCapacity), pid) for pid in pids],
            RemoteChannel(() -> PESlotChannel(queueCapacity), myid()),
