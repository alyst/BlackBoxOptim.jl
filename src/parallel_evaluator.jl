...
typealias ParallelEvaluatorWorkerRef{T,P} ChannelRef{ParallelEvaluatorWorker{T,P}}

@inline @compat function (worker::ParallelEvaluatorWorker)()
    put_fitness!(worker.shared_fitness, fitness(worker.shared_param, worker.problem))
    return nothing
end

@inline @compat function (worker_ref::ParallelEvaluatorWorkerRef{T,P}){T,P}() =
...
type ParallelEvaluator{F, FA, T, FS, P<:OptimizationProblem, A<:Archive} <: Evaluator{P}
    ...
    @compat (::Type{ParallelEvaluator}){P<:OptimizationProblem, A<:Archive}(
        ...
        worker_refs = [@compat RemoteChannel(function ()
