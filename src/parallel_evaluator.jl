...
immutable ParallelEvaluatorWorker{P<:OptimizationProblem}
    ...
    @compat (::Type{ParallelEvaluatorWorker}){P<:OptimizationProblem}(id::Int, problem::P,
...
type ParallelEvaluator{F, FA, FS, P<:OptimizationProblem, A<:Archive} <: Evaluator{P}
    ...
    @compat (::Type{ParallelEvaluator}){P<:OptimizationProblem, A<:Archive}(
        ...
        etor = new{F, FA, typeof(fs), P, A}(
            ...
            PESlotChannelRef[@compat RemoteChannel(() -> PESlotChannel(paramChanCapacity), pid) for pid in pids],
            @compat RemoteChannel(() -> PESlotChannel(queueCapacity), myid()),
