const ParallelPopulationOptimizer_DefaultParameters = ParamsDict(
...
typealias WorkerChannelRef{F} ChannelRef{CandidateMessage{F}}
...
type ParallelPopulationOptimizer{F, P<:OptimizationProblem} <: SteppingOptimizer
  ...
  @compat final_fitnesses::Vector{Future}       # references to the @spawnat ID run_worker()
  ...
  is_started::ChannelRef{Bool}                  # flag that all workers have started
...
function store!{F}(ppe::ParallelPseudoEvaluator{F}, msg::CandidateMessage{F})
  ...
  (metrics.num_sent == metrics.num_sent_prev+1) ||
...
# If running, its channel refs should not be ready,
# but if there was exception in the worker,
# it would be thrown into the main thread
function check_worker_running(worker::@compat(Future))
  if isready(worker)
...
function ParallelPopulationOptimizer{P<:OptimizationProblem}(
    problem::P, optimizer_generator::Function;
    NWorkers::Int = 1, Workers::Vector{Int} = Vector{Int}(),
  ...
  ParallelPopulationOptimizer{F, P}(Workers, optimizer_generator, MigrationPeriod, MigrationSize,
        @compat(Vector{Future}(length(Workers))),
        @compat(RemoteChannel(() -> WorkerChannel{F}(FromWorkersChannelCapacity))),
        WorkerChannelRef{F}[@compat RemoteChannel(() -> WorkerChannel{F}(ToWorkerChannelCapacity), id) for id in Workers],
        @compat(RemoteChannel(() -> Channel{Bool}(1))),
...
function setup!(ppopt::ParallelPopulationOptimizer)
  ...
  @compat workers_ready = RemoteChannel(() -> Channel{Int}(nworkers(ppopt))) # FIXME do we need to wait for the worker?
...
function redirect{F}(ppopt::ParallelPopulationOptimizer{F}, msg::CandidateMessage{F})
  ...
  msg.candi.extra = NO_GEN_OP # reset operation and tag to avoid calling adjust!() out-of-context
  msg.candi.tag = 0
...
function shutdown!{F}(ppopt::ParallelPopulationOptimizer{F})
  if !isready(ppopt.is_started)
    info("Parallel optimizer has not started")
    return
  end
  ...
  for i in eachindex(ppopt.final_fitnesses)
    ...
    msg::CandidateMessage{F} # this is not declaration, this is type assertion
    @assert msg.worker == i
...
type PopulationOptimizerWrapper{F,O<:PopulationOptimizer,E<:Evaluator}
  ...
  @compat function (::Type{PopulationOptimizerWrapper}){F,O,E}(
